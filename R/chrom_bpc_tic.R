...
chrom_bpc_tic <- function(df, pars) {
    ...
    pa <- plot_ly(source = "pa") %>% add_trace(data = tic, x = ~scantime, 
        y = ~V1, type = "scatter", mode = "lines", line = list(color = "rgba(0, 0, 0,0.7)", 
            width = 0.8), text = ~paste(scantime, "s"), name = "<b>Total ion chromatogram</b>") %>% 
        add_trace(data = bpc, x = ~scantime, y = ~V1, type = "scatter", 
            mode = "lines", line = list(color = "rgba(0, 215, 167,1)", 
                width = 1.1), text = ~paste(round(V2, 4), "m/z"), name = "<b>Base peak chromatogram</b>") %>% 
