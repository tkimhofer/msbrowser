...
#' @import shiny
#' @importFrom shinyjs hide hideElement show showElement toggle toggleElement
...
server <- function(input, output, session) {
  ...
  # hide tabs at the start
  observe({
    hideTab(inputId = "msexpl", target = "ichron")
    hideTab(inputId = "msexpl", target = "rawData")
    hideTab(inputId = "msexpl", target = "ppick")
    hideTab(inputId = "msexpl", target = "peaks")
    hide(id='run1')
    shinyjs::hide(id = "summary_file")
    shinyjs::hide(id = "div_target")
    shinyjs::hideElement(id = "div_vis")
    shinyjs::hide(id = "div_xic")
    shinyjs::hide(id = "div_ppick")
  })

  # expand fields for manual entering mz values for xic
  observeEvent(input$imp_xic,{
    shinyjs::toggle(id = "div_xic")
  ...
  observeEvent(input$go_xic, {
    ...
    if(all(is.numeric(xic_ra))){
      ...
      if(length(xic_ra)==2 & xic_ra[1]>=mz_ra[1] & xic_ra[2]<=mz_ra[2]){
  ...
  # SELECT FILE
  {
    ...
    observeEvent(input$fileexample, {
      ...
      msfile('/Volumes/Torben_2/testData/AutismW_HPOS_ToF10_B1E1_SR.mzML')
    }, ignoreNULL = T, ignoreInit = T)
  }


  # PROCESS DATA
  raw_data<-eventReactive(msfile(), {
    showTab(inputId = "msexpl", target = "ichron")
    shinyjs::show(id = "summary_file")
    message('Reading file')
    raw_xcms=xcmsRaw(msfile(), profstep = 0, includeMSn = F, mslevel = 1)
    # extract raw data from xcms object
    df_xcms=data.frame(mz=raw_xcms@env$mz, Int=raw_xcms@env$intensity)
    # extract intensities p scan and scatime
    ...
    noise98p=round(quantile(df_xcms$Int,probs = 0.98))
    ...
    message('Read-in completed')
    #browser()
    df=df_xcms
    tic=ddply(df, .(scantime),function(x) sum(x$Int))
    bpc=ddply(df, .(scantime),function(x) {idx=which.max(x$Int)[1]; c(x$Int[idx], x$mz[idx])})
    # Get maximumn signal and scantime of that signal
    xic_mz=df$mz[which.max(df_xcms$Int)]
    xic_scant=df$scantime[which.max(df_xcms$Int)]
    return(list(df_xcms, raw_xcms, xic_mz, xic_scant, noise98p, mzra=range(df$mz)))
  }, ignoreNULL = T, ignoreInit = T)
  ...
  # GENERATE CHROMATOGRAMS AND SINGLE SCAN MASS SPECTRUM
  {
    ...
    xic_ra=reactiveVal(NULL)
    massSpec_scantime=reactiveVal(NULL)

    # TRIGGERS
    # 1. new data
    observeEvent(raw_data(), {
      # scantime for single scan mass spectrum
      massSpec_scantime(raw_data()[[4]])
      # mzrange for xic
      xic_mz=raw_data()[[3]]
      ppm_change=50
      ...
      xic_ra(c(low, high, xic_mz))
    }, ignoreNULL = T)

    # 2. click on BPC/TIC
    observeEvent(event_data("plotly_click", source='pa'), {
      df=raw_data()[[1]]
    ...
    output$xic <- renderPlotly({
      ...
      xic=ddply(ds, .(scantime), function(x) sum(x$Int))
      ...
      if(length(xic_ra())==3){
        tit=paste0('<b>Extracted ion chromatogram</b> m/z ', round(xic_ra()[3], 4),
                   ' (&plusmn;', ppm_change/2, ' ppm)<br>(Exact mass range:',
    ...
    # update mz values in
    observeEvent(raw_data(),{
      updateNumericInput(session, inputId='in_mz', value=round(raw_data()[[3]], 2))
      updateNumericInput(session, inputId='in_rt', value=round(raw_data()[[4]]))
      updateNumericInput(session, 'in_noisethr', value=as.numeric(raw_data()[[5]]))
      output$selection <- renderText({
        paste('Signal:', round(raw_data()[[4]]), 's', round(raw_data()[[3]], 2), 'm/z')
      })
    })
  }
  ...
  # internal standards / db entries
  {
    observeEvent(input$"db_assays", {
      ...
      updateSelectizeInput(session, "in_icst",
                           label = "Compound",
                           choices = dlply(icst_cname, .(info), function(x) {x[,2]}),
                           selected=F)
  ...
  #
  observeEvent(event_data("plotly_click", source='pc')$x,{
    updateRadioButtons(session, 'target_input',  selected='click')
    dats=event_data("plotly_click", source='pc')
    output$selection <- renderText({
  ...
  ttt=eventReactive(input$goImage, {
    ...
    if(indic()==0) showElement(id='move1')
    return(sub)
  })
  ...
  fgt <- eventReactive(input$pickpeak1, {
    ...
    if(length(idx)>0){
      ...
      output$pp1 <- renderPlotly({
        ...
        idc=unlist(dlply(ptbl, .(roi), function(peak, ds=mf){
      ...
      showNotification(ui="No peaks detected!", duration=NULL, closeButton = T, type='error', id='nopeaks'); return(NULL)
      }
  }, ignoreNULL = T)
